...
create table if not exists public.planning_passwords (
  id bigint generated by default as identity primary key,
  planning_id text not null references public.planning_state(id) on delete cascade,
  role text not null,
  password text not null,
  created_at timestamptz not null default now(),
  constraint planning_passwords_planning_role_key unique (planning_id, role)
);
